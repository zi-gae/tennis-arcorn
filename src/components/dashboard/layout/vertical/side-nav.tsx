...
function renderNavItems({
	...
	const children = items.reduce((acc: React.ReactNode[], curr: NavItemConfig): React.ReactNode[] => {
		...
		acc.push(
			<NavItem key={key} itemKey={key} depth={depth} forceOpen={forceOpen} pathname={pathname} {...item}>
...
function NavItem({
	...
	// Handle the special case for "members-me" to use the current user's ID
	const actualHref = itemKey === "members-me" && user?.id && href ? href.replace(":id", user.id) : href;
